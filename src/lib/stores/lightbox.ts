...
export const lightbox = {
    ...
    next: () => store.update((state) => ({ ...state, currentIndex: (state.currentIndex + 1) % state.images.length })),
    prev: () => store.update((state) => ({ ...state, currentIndex: (state.currentIndex - 1 + state.images.length) % state.images.length }))
};