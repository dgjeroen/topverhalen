...
type ProjectTheme = Record<string, string>;

export interface StoryBlock {
	id: string; // Elk blok heeft wss een ID
	type: string; // En een type (bv. 'text', 'image')
	// Index signature: sta extra properties toe, maar noem ze 'unknown'
	[key: string]: unknown;
}
...
export interface ProjectContent {
	...
	// ✅ FIX 1: Specifiekere definitie voor het thema
	theme: ProjectTheme;

	// ✅ FIX 2: Array van getypeerde blokken i.p.v. 'any[]'
	data: StoryBlock[];
}
...
export async function getProjectsIndex(): Promise<ProjectIndex> {
	...
	const response = await fetch(
		`https://api.github.com/gists/${INDEX_GIST_ID}?cache_buster=${timestamp}`,
		{
			headers: {
...
export async function getGist(id: string): Promise<ProjectContent> {
	...
	const response = await fetch(`https://api.github.com/gists/${id}?cache_buster=${timestamp}`, {
		headers: {
			Authorization: `token ${GITHUB_TOKEN}`,
			Accept: 'application/vnd.github.v3+json'
		}
...
export async function updateGist(id: string, content: ProjectContent): Promise<UpdateResult> {
	...
	for (let attempt = 0; attempt < maxRetries; attempt++) {
		try {
			...
			const usagePercent =
				((lastRateLimit.limit - lastRateLimit.remaining) / lastRateLimit.limit) * 100;
			if (usagePercent > 80) {
				console.warn(
			...
			if (response.status === 409) {
				...
				console.warn(
					`⚠️ Gist conflict (attempt ${attempt + 1}/${maxRetries}), retrying in ${delay}ms...`
				);
